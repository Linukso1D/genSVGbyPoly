...
public class Polygon implements Cloneable, Serializable {
	...
	public String toSvg() {
		...
		Points.forEach((Point p) -> points.add(p.X + "," + p.Y));
		String arrayPoints = String.join(" ", points);
		String color = "style=\"fill:rgba(" + Brush.R + "," + Brush.G + "," + Brush.B + "," + ((double) Brush.A / 255) + ")\"";
		return " <polygon points=\"" + arrayPoints + "\" " + color + " />";
	}
}
